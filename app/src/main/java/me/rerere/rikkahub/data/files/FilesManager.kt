...
class FilesManager(
     ...
     * upload/ 目录下现存的文件名快照。
     *
     * 系统自动备份不包含 upload/ (见 res/xml/backup_rules.xml), 所以卸载重装后
     * 数据库会带着图片路径还原, 但文件本身已经不在。加载消息时用这份快照判断
     * 图片是否还存在, 一次列目录就够, 不必对每张图单独 stat 磁盘。
     */
    fun listExistingUploadFileNames(): Set<String> {
        val dir = context.filesDir.resolve(FileFolders.UPLOAD)
        if (!dir.isDirectory) return emptySet()
        return dir.list()?.toSet() ?: emptySet()
    }
    ...
    /**
     * 生图结果落地: 上游只给远端 URL 时图片已经被下载成临时文件, 这里直接搬过去而不是
