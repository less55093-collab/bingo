...
class SettingsStore(
    ...
    companion object {
        ...
        // 首启动欢迎动画是否已播放过
        val WELCOME_ANIMATION_SHOWN = booleanPreferencesKey("welcome_animation_shown")
    ...
    val settingsFlowRaw = dataStore.data
        .catch { exception ->
            ...
            Settings(
                ...
                imageGenerationModelId = preferences[IMAGE_GENERATION_MODEL]?.let { Uuid.parse(it) }
                    ?: BINGO_IMAGE_MODEL_ID,
                titlePrompt = preferences[TITLE_PROMPT] ?: DEFAULT_TITLE_PROMPT,
                translatePrompt = preferences[TRANSLATION_PROMPT] ?: DEFAULT_TRANSLATION_PROMPT,
                translateThinkingBudget = preferences[TRANSLATE_THINKING_BUDGET] ?: 0,
                suggestionPrompt = preferences[SUGGESTION_PROMPT] ?: DEFAULT_SUGGESTION_PROMPT,
                ocrModelId = preferences[OCR_MODEL]?.let { Uuid.parse(it) } ?: DEFAULT_AUTO_MODEL_ID,
                ...
                welcomeAnimationShown = preferences[WELCOME_ANIMATION_SHOWN] == true,
        ...
        .map {
            ...
            // 只保留一个内置助手: 移除老版本残留的第二个默认助手(仅当用户没有改动过它)
            assistants.removeAll { assistant ->
                assistant.id in LEGACY_DEFAULT_ASSISTANT_IDS && assistant.isPristineLegacyDefault()
            }
    ...
    suspend fun update(settings: Settings) {
        ...
        dataStore.edit { preferences ->
            ...
            preferences[WELCOME_ANIMATION_SHOWN] = settings.welcomeAnimationShown
...
data class Settings(
    ...
    val welcomeAnimationShown: Boolean = false,
    ...
    val translateModeId: Uuid = BINGO_FAST_MODEL_ID,
    val translatePrompt: String = DEFAULT_TRANSLATION_PROMPT,
    val translateThinkingBudget: Int = 0,
    val enableSuggestion: Boolean = true,
    val suggestionModelId: Uuid? = null,
    val suggestionPrompt: String = DEFAULT_SUGGESTION_PROMPT,
    val ocrModelId: Uuid = BINGO_DEFAULT_MODEL_ID,
    val ocrPrompt: String = DEFAULT_OCR_PROMPT,
    val compressModelId: Uuid = BINGO_FAST_MODEL_ID,
...
data class DisplaySetting(
    ...
    // 默认开启：后台生成完成后通知用户是预期行为，关掉的话用户离开页面就完全收不到反馈。
    // 无 POST_NOTIFICATIONS 权限时 NotificationUtil.notify 会静默返回 false，因此默认开启是安全的。
...
/**
 * 判断老版本第二个默认助手是否从未被用户改动过。
 * 只有完全等于出厂状态时才会在迁移中被删除, 避免丢失用户的自定义配置。
 */
private fun Assistant.isPristineLegacyDefault(): Boolean {
    val factory = LEGACY_PRISTINE_DEFAULT_ASSISTANT.copy(id = this.id)
    return this == factory
}
...
/**
 * 早期版本内置了两个默认助手, 现在只保留一个。这个 id 之前属于第二个助手,
 * [LEGACY_DEFAULT_ASSISTANT_IDS] 用于在迁移时清理没有被用户改动过的残留助手。
 */
private val LEGACY_SECOND_DEFAULT_ASSISTANT_ID = Uuid.parse("3d47790c-c415-4b90-9388-751128adb0a0")

internal val LEGACY_DEFAULT_ASSISTANT_IDS = listOf(LEGACY_SECOND_DEFAULT_ASSISTANT_ID)

/** 老版本第二个默认助手的出厂状态, 用于判断用户是否改动过它 */
private val LEGACY_PRISTINE_DEFAULT_ASSISTANT = Assistant(
    id = LEGACY_SECOND_DEFAULT_ASSISTANT_ID,
    name = "",
    systemPrompt = """
