...
fun ChatPage(id: Uuid, text: String?, files: List<Uri>, nodeId: Uuid? = null) {
    ...
    // 后台生成完成通知默认开启，但 POST_NOTIFICATIONS 在无权限时会被静默丢弃。
    // 权限必须在退到后台之前就拿到，所以进入聊天页时请求一次，而不是等到生成结束。
    ...
    val wantsNotification = setting.displaySetting.enableNotificationOnMessageGeneration ||
        setting.displaySetting.enableLiveUpdateNotification
    LaunchedEffect(wantsNotification) {
