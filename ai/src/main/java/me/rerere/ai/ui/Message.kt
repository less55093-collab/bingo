                    ...
                    is UIMessagePart.Reasoning -> {
                        ...
                        if (deltaPart.reasoning.isEmpty() && deltaPart.metadata == null) {
                            ...
                            if (lastPart is UIMessagePart.Reasoning) {
                                ...
                                // Create new Reasoning part, but inherit createdAt from an
                                // earlier still-unfinished Reasoning part if one exists:
                                // 思考被其他 part 隔开时不能重新起算, 否则计时器归零重启
                                val ongoing = acc.lastOrNull {
                                    it is UIMessagePart.Reasoning && it.finishedAt == null
                                } as? UIMessagePart.Reasoning
                                if (ongoing != null) {
                                    acc + deltaPart.copy(createdAt = ongoing.createdAt)
                                        .also { it.metadata = deltaPart.metadata }
                                } else {
                                    acc + deltaPart
                                }
                            }
