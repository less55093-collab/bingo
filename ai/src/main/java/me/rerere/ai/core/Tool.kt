...
data class Tool(
    ...
    /** Optional access to the current conversation when a tool needs user attachments. */
    val executeWithContext: suspend (JsonElement, List<UIMessage>) -> List<UIMessagePart> = { args, _ ->
        execute(args)
    },
)
