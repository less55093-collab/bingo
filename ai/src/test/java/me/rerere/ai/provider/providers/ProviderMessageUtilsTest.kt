...
class ProviderMessageUtilsTest {
    ...
    @Test
    fun `image generation output is summarized for the model without mutating UI output`() {
        val tool = UIMessagePart.Tool(
            toolCallId = "image-call",
            toolName = "generate_image",
            input = "{}",
            output = listOf(
                UIMessagePart.Image("file:///generated-image.png"),
                UIMessagePart.Text("The original tool result should remain local."),
            ),
        )
        ...
        assertTrue(tool.output[0] is UIMessagePart.Image)
        assertEquals("The original tool result should remain local.", (tool.output[1] as UIMessagePart.Text).text)
    }

    @Test
    fun `non image generation tools keep their original output for the model`() {
        val output = listOf(UIMessagePart.Image("file:///tool-image.png"))
        val tool = UIMessagePart.Tool("tool-call", "other_tool", "{}", output)

        assertEquals(output, tool.outputForModel())
    }
